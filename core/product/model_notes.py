...
class ProductLine(models.Model):
    ...
    order = OrderField(unique_for_field="product", blank=True)  # custom fild
    # the models prefix is not needed because this custom field extends the models class
    # we're running our query on the unique_for_field="product" so whenever we grab
    # or generate a ordering num that ordering num is going to be related to only sub
    # products related to a particular product
    is_active = models.BooleanField(default=False)
